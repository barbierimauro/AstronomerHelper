...
for year in range(1994,2022):
	for month in range(1,13):
		...
		vec_t2.append(t_end)
		query_meteo_single="""SELECT start_date,press_2m,rhum_2m,temp_2m,tempdew_2m,temp_30m,wind_dir_30m,wind_speed_30m,wind_dir_10m,wind_speed_10m FROM asm.meteo_lasilla WHERE start_date > '"""+vec_t1[i]+"""' AND start_date<'"""+vec_t2[i]+"""'"""
		#print(query_meteo_single)
		query_meteo.append(query_meteo_single)
...
for nm_ in range(nmonths):
	...
	t1 = timelib.time()
	if(FULLQUERY):
		rawframes = tapobs.search(query=query_meteo[nm_],maxrec=MAXRECORDS)
	else:
		rawframes = tapobs.search(query=query_meteo[nm_],maxrec=SMALLREC)
	t2 = timelib.time()
